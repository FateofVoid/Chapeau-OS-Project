...
namespace ChapeauDAL
{
    public class Order_DAO : Base
    {
        ...
        public void Db_Insert_Order(Order order/*int orderId, DateTime dateTime, int status, decimal tip, string feedback*/)
        {
            ...
            string query = String.Format("INSERT INTO Orders VALUES({0}, '{1}', '{2}', {3}, {4}, '{5}')",
                order.id, date, time, order.state, order.tip, order.feedback);
            SqlParameter[] sqlParameters = new SqlParameter[0];
            ExecuteEditQuery(query, sqlParameters);
        }
        ...
        //***********************************
        //Payment
        public List<Item> MakeOrderItemsList(int orderId)
        {
            List<Item> items = new List<Item>();
            string query = $"SELECT * FROM MenuItem WHERE Id={FetchOrderItemsIds(orderId)} ";
            SqlParameter[] sqlParameters = new SqlParameter[0];
            DataTable datatable = (ExecuteSelectQuery(query, sqlParameters));
            foreach (DataRow dr in datatable.Rows)
            {
                Item item = new Item
                {
                    ...
                    Type = (string)(dr["Type"].ToString()),
                    Sub_Type = (string)(dr["Sub_Type"].ToString())
                };
                items.Add(item);
        ...
        {
            string query = $"UPDATE Orders SET Status={2}, Tip={tip} WHERE Id={orderId};";
            SqlParameter[] sqlParameters = new SqlParameter[0];
            ExecuteEditQuery(query, sqlParameters);
        }
